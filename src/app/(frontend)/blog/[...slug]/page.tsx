...
import { IMAGE_QUERY, MODULES_QUERY, TRANSLATIONS_QUERY } from '@/sanity/lib/queries'
...
async function getPost(params: Params) {
  ...
  return await fetchSanityLive<Sanity.BlogPost & { modules: Sanity.Module[] }>({
    query: groq`*[
			...
			'headings': body[style in ['display-hero', 'display-xxl', 'display-xl', 'h1', 'h2', 'h3']]{
				style,
				'text': pt::text(@)
			},
			categories[]->,
...
function processSlug(params: Params) {
  const lang = languages.includes(params.slug[0] as Lang) ? params.slug[0] : undefined
